...
Codex 会话工具是一款面向 Windows 的本地桌面工具。它扫描保存在 `CODEX_HOME` 中的历史会话，并将全部会话或用户选中的会话恢复到当前 Provider。

[下载页面](https://github.com/XVibeCoding/codex-session-hub/releases)
...
2. 从 [Releases](https://github.com/XVibeCoding/codex-session-hub/releases) 下载 Windows EXE 或安装包。
3. 打开 Codex 会话工具，等待自动扫描完成。
...
`npm run dev` 只启动 Vite 前端页面，不包含 Rust core 和 Tauri IPC，不能用于验证真实会话修复流程。
...
```text
src-tauri/target/release/codex-session-hub.exe
src-tauri/target/release/codex-session-hub-cli.exe
...
<p align="center">扫码加入 Codex 会话工具 交流群：</p>

<p align="center">
  <img src="asserts/qq交流群.jpg" alt="Codex 会话工具 QQ 交流群二维码" width="320" />
</p>
